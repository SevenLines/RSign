...
export_end:
    ...
    if ( ERROR_WAS ) {
        if (!fWickedErrorWas) {
          ShowMessage("Во время исполнения были обнаруженны небольшие ошибки. Рекомендуется ознакомиться с журналом.");
        } else {
          ShowMessage("Во время исполнения были обнаруженны серьезные ошибки. Возможно что некотороые строки не были выведены.\nРекомендуется ознакомиться с журналом.");
        }
    }
    aexp->EndDocument();
