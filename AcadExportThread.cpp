...
{
    SET_PROGRESS_FORM_POSITION(0;)
    aexp->AddLayer("RoadCityObjects");
    for (int i=0;i<data->Objects->Count;i++) {
        if (Terminated) return -1;
        if (data->Objects->Items[i]->DictId==75 || data->Objects->Items[i]->DictId==76) {
            SET_PROGRESS_FORM_POSITION(i;)
...
int __fastcall AcadExportThread::ExportDefects(TDtaSource* data, TAcadExport* aexp)
{
    SET_PROGRESS_FORM_POSITION(0)
    aexp->AddLayer("RoadDefects");

    for (int i=0;i<data->Objects->Count;i++) {
        if (Terminated) return -1;
        SET_PROGRESS_FORM_POSITION(i)
        if (data->Objects->Items[i]->DictId==60) {
            TRoadDefect *defect = dynamic_cast<TRoadDefect*>(data->Objects->Items[i]);
            if (defect) {
                TExtPolyline *p = defect->PrepareMetric(R);
                aexp->ExportDefect(p, defect);
                delete p;
            }
        }
    }
    aexp->ExportDefect(0, 0, true);
    return 0;
}
...
#define EXPORT_ITEM(function, message) \
    ...
    if (aexp->BeginDocument(R)) {
        ...
        if (MetricData) {
            ...
            if (FAutoCADExport->ExportDefects) {
                EXPORT_ITEM(ExportDefects(MetricData, aexp), "Выводим дефекты...");
            }
        }
