...
// «десь можно написать двоичный поиск.
for (i=0;i<n;i++)     // i - номер первой точки в области вывода
   if (L[i]>=L1)
      break;

for (j=i;j<n;j++)       // j - перва€ точка за областью вывода
   if (L[j]>L2)
      break;

if ((i==n)||(j==0))
    return;         // “очки начинаютс€ за областью вывода
SetCount(j-i+2);
int s=0;
if (i>0)
    {
...
class TCurvePoint {
public :
   double bR; // ќбратный радиус кривизны в точке
