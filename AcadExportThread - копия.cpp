...
void __fastcall AcadExportThread::Execute()
{
    ...
    if(aexp->BeginDocument(R)) {
      ...
      try{
          ...
          /* -- Подсчет разметки от осевой -- */
           ...
           if(Data&&(!FAutoCADExport->TableRowLinesBottom||!FAutoCADExport->TableRowLinesTop)){
         ...
         if(DataCur) {
           ...
           if (FAutoCADExport->ExportPlan) {
             ...
             for (int i=0;i<CurData->Objects->Count;i++) {
                if(Terminated) return;
                if (CurData->Objects->Items[i]->DictId==ROADPLANCODE) {
                   SET_PROGRESS_FORM_POSITION(i;)
           ...
           if (FAutoCADExport->ExportSurface) {
             ...
             for (int i=0;i<CurData->Objects->Count;i++) {
                if(Terminated) return;
                if (CurData->Objects->Items[i]->DictId==ROADSURFACECODE) {
                   SET_PROGRESS_FORM_POSITION(i;)
                   TRoadPart *t=dynamic_cast<TRoadPart*>(CurData->Objects->Items[i]);
                   if (t) {
                      TExtPolyline *p=t->PrepareMetric(R);
         ...
         if ((CurData||PrjData) && MetricData) {
           if(FAutoCADExport->ExportRoadSigns){
             ...
             if(PrjData) {
               for (int i=0;i<PrjData->Objects->Count;i++) {
                  if(Terminated) return;
                  if (PrjData->Objects->Items[i]->DictId>=SGNCODE && PrjData->Objects->Items[i]->DictId<SGNCODE+10) {
                     TRoadSign *t=dynamic_cast<TRoadSign*>(PrjData->Objects->Items[i]);
                     if (t) {
                        TExtPolyline *p=t->PrepareMetric(R);
             ...
             if(!PrjData&&CurData) {
               for (int i=0;i<CurData->Objects->Count;i++) {
                  if(Terminated) return;
                  if (CurData->Objects->Items[i]->DictId>=SGNCODE && CurData->Objects->Items[i]->DictId<SGNCODE+10) {
                     TRoadSign *t=dynamic_cast<TRoadSign*>(CurData->Objects->Items[i]);
                     if (t) {
                        TExtPolyline *p=t->PrepareMetric(R);
                        A.push_back(wpsign(t,p));
                     }
           ...
           if(PrjData) {
             if(FAutoCADExport->ExportMark){
               ...
               for (int i=0;i<PrjData->Objects->Count;i++) {
                  if(Terminated) return;
                  if (PrjData->Objects->Items[i]->DictId==ROADMARKCODE) {
                     SET_PROGRESS_FORM_POSITION(i);
                     TRoadMark *t=dynamic_cast<TRoadMark*>(PrjData->Objects->Items[i]);
                     if (t) {
                        TExtPolyline *p=t->PrepareMetric(R);
                        TPlanLabel *l=t->GetText(0,R,Dict);
                        String s=l->Caption.c_str();
           ...
           if(CurData) {
             if(FAutoCADExport->ExportMark){
               ...
               for (int i=0;i<CurData->Objects->Count;i++) {
                  if(Terminated) return;
                  if (CurData->Objects->Items[i]->DictId==ROADMARKCODE) {
                     SET_PROGRESS_FORM_POSITION(i);
                     TRoadMark *t=dynamic_cast<TRoadMark*>(CurData->Objects->Items[i]);
                     if (t) {
                        TExtPolyline *p=t->PrepareMetric(R);
                        TPlanLabel *l=t->GetText(0,R,Dict);
                        String s=l->Caption.c_str();
        ...
        /* Трубы, мосты ,автобусные остановки и высоты насыпей
           выводим из существующих данных  */
        ...
        if (CurData) {
           ...
           if (FAutoCADExport->ExportCommunications) {
             ...
             for (int i=0;i<CurData->Objects->Count;i++) {
                if(Terminated) return;
                if (CurData->Objects->Items[i]->DictId==COMMUNICATIONCODE) {
                   SET_PROGRESS_FORM_POSITION(i;)
                   TCommunication *t=dynamic_cast<TCommunication*>(CurData->Objects->Items[i]);
                   if (t) {
                      TExtPolyline *p=t->PrepareMetric(R);
           ...
           if(FAutoCADExport->ExportTubes){
             ...
             for (int i=0;i<CurData->Objects->Count;i++) {
                if(Terminated) return;
                if (CurData->Objects->Items[i]->DictId==ROADTUBECODE) {
                   SET_PROGRESS_FORM_POSITION(i;)
                   TRoadTube *t=dynamic_cast<TRoadTube*>(CurData->Objects->Items[i]);
                   if (t) {
                      TExtPolyline *p=t->PrepareMetric(R);
                      aexp->ExportTube(p,t);
                      delete p;
           ...
           if(FAutoCADExport->ExportBridges){
             ...
             for (int i=0;i<CurData->Objects->Count;i++) {
                if(Terminated) return;
                if (CurData->Objects->Items[i]->DictId==ROADBRIDGECODE) {
                   SET_PROGRESS_FORM_POSITION(i;)
                   TRoadBridge *t=dynamic_cast<TRoadBridge*>(CurData->Objects->Items[i]);
                   if (t) {
                      TExtPolyline *p=t->PrepareMetric(R);
           ...
           if(FAutoCADExport->ExportBusstops){
             ...
             for (int i=0;i<CurData->Objects->Count;i++) {
                if(Terminated) return;
                if (CurData->Objects->Items[i]->DictId==BUSSTOPCODE) {
                   SET_PROGRESS_FORM_POSITION(i;)
                   TBusStop *t=dynamic_cast<TBusStop*>(CurData->Objects->Items[i]);
                   if (t) {
                      TExtPolyline *p=t->PrepareMetric(R);
           ...
           if (FAutoCADExport->ExportMoundHeights) {
               ...
               for(int fase =0;fase <2;fase++){
                 for (int i=0;i<CurData->Objects->Count;i++) {
                     if(Terminated) return;
                     if (CurData->Objects->Items[i]->DictId==MOUNDHEIGHTCODE) {
                        if(fase) SET_PROGRESS_FORM_POSITION(i);
                        TMoundHeight *m=dynamic_cast<TMoundHeight*>(CurData->Objects->Items[i]);
                        if (m) {
           ...
           // Выводим ограждения
           /*if(FAutoCADExport->ExportBarriers){
             SET_PROGRESS_FORM_CAPTION("Выводим ограждения...")
             SET_PROGRESS_FORM_POSITION(0;)
             Application->ProcessMessages();
             for (int i=0;i<MetricData->Objects->Count;i++) {
                SET_PROGRESS_FORM_POSITION(i;)
                if (MetricData->Objects->Items[i]->DictId==ROADBARRIERCODE) {
                   TRoadBarrier *t=dynamic_cast<TRoadBarrier*>(MetricData->Objects->Items[i]);
                   if (t) {
                      TExtPolyline *p=t->PrepareMetric(R);
                      aexp->ExportBarrier(p,t);
                      delete p;
                   }
                }
             }
           ...
           if(FAutoCADExport->ExportSignal || FAutoCADExport->ExportSidewalks ||
             ...
             if (CurData && CurData!=PrjData) {
                ...
                SET_PROGRESS_FORM_MINMAX(0,CurData->Objects->Count-1);
                SET_PROGRESS_FORM_POSITION(0)
                bool fIncludeSignalExist = FAutoCADExport->ExportSignalExistToo;
                for (int i=0;i<CurData->Objects->Count;i++) {
                    SET_PROGRESS_FORM_POSITION(i)
                    if(Terminated) return;
  ...
  /*
             ...
             for (int i=0;i<MetricData->Objects->Count;i++) {
                SET_PROGRESS_FORM_POSITION(i;)
                if (MetricData->Objects->Items[i]->DictId==ROADSIGNALCODE) {
                   TRoadSignal *t=dynamic_cast<TRoadSignal*>(MetricData->Objects->Items[i]);
                   if (t) {
                      TExtPolyline *p=t->PrepareMetric(R);
                      aexp->ExportSignal(p,t);
                      delete p;
                   }
                }else if (MetricData->Objects->Items[i]->DictId==ROADBARRIERCODE) {
                   TRoadBarrier *t=dynamic_cast<TRoadBarrier*>(MetricData->Objects->Items[i]);
                   if (t) {
                      TExtPolyline *p=t->PrepareMetric(R);
                      aexp->ExportBarrier(p,t);
           ...
           // Выводим тротуары и площадки отдыха
           /*if(FAutoCADExport->ExportRoadSideObjects){
             SET_PROGRESS_FORM_CAPTION("Выводим тротуары и площадки отдыха...")
             SET_PROGRESS_FORM_POSITION(0;)
             aexp->AddLayer("RoadSideObjects");
             ...
             for (int i=0;i<MetricData->Objects->Count;i++) {
                SET_PROGRESS_FORM_POSITION(i;)
                if(Terminated) return;
                if (MetricData->Objects->Items[i]->DictId==SIDEWALKCODE|| MetricData->Objects->Items[i]->DictId==RESTZONECODE){
                   TSquareRoadSideObject_Kromka *t=dynamic_cast<TSquareRoadSideObject_Kromka*>(MetricData->Objects->Items[i]);
                   if (t) {
                      TExtPolyline *p=t->PrepareMetric(R);
                      if (t->DictId==SIDEWALKCODE)
                         aexp->ExportSidewalk(p,t);
                      else if (t->DictId==RESTZONECODE)
                         aexp->ExportRestZone(p,t);
                      delete p;
                   }
                }
