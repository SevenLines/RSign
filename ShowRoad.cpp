...
void __fastcall TRoadFrm::CalculateVisibility(void) {
    if (FProfilData && FEditedData) {
        double DriverHeight=1.0; //Высота водителя (в метрах)
        double MarkerHeight=1.0; //Высота встречного препятствия (атовмобиля)
        int PartLen=5000; // Длина участка на котором определяется видимость (в сантиметрах)
        const int StepLen=100; // Шаг измерения видимости
        ...
        const int Diskrete=1000;//Шаг округления видимости в см.
        const int MaxVis=30000;//Максимальная видимость
        ...
        for (int dir=1;dir>=-1;dir-=2) {
          int spos=StartPos;
          int epos=EndPos;
          int stat[StatSize];    // Массив со статистикой по интервалу
          int count=0;
          ...
          for (;spos<epos;spos+=PartLen) {
             for (int i=0;i<StatSize;i++)
                stat[i]=0;
             double est=0,disp=0;// матожидание и дисперсия
             ...
             if (stat[StatSize-1]<fcount) {
               ...
               disp=disp/(mcount-stat[StatSize-1]-1)-est*est; //несмещенная дисперсия
               ...
               int vislen;
               if (cnt==0) // Такое маловероятно но надо что-то делать
