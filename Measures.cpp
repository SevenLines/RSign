...
TVisibility *__fastcall TRoadGeometry::CalculateVisibility(int spos,int epos,double DriverHeight,double MarkerHeight,int dir) {
  DriverHeight*=100; // ¬се в сантиметрах
  MarkerHeight*=100;
...
{
int Res=1;
// ѕровер€ем, надо ли разбивать участок
int i=Node->LeftInd;
int j=Node->RightInd;
__int32 LeftL=L[i];
__int32 RightL=L[j];
while ((i<j)&&(L[i]-LeftL<MinLen))  // ѕробегаем рассто€ние MinLen слева
    i++;
while ((i<j)&&(RightL-L[j]<MinLen))  // ѕробегаем MinLen справа
    j--;
...
if ((Max1-Min1>Dev)||(Max2-Min2>Dev))   // Ќадо разбивать участки
    {
    int MaxD=0;
    int DivPos=0;
    for (int k=i;k<=j;k++)
...
{
if (Node)
    if ((Node->LeftNode==NULL)&&(Node->RightNode==NULL))  // “олько листь€
        {
        int n=Node->RightInd;
        int SlopeNorm=0;
...
double SlopeNorm=0;
int MidSlopeVal;
// —читаем средний уклон на ровно MinLen метров после точки
// ≈сли точка близко к краю то расто€ние меньше
int i,j;// i - начало полоски, j- конец полоски
for (i=0,j=1;i<FCount-1;i++)
    {
    while (j<FCount)  // Ќабираем длину из отрезков
        {
        int DL=L[j]-L[j-1];
        if (CurLen+DL>MinLen)
            break;
...
// —читаем средний уклон на ровно MinLen метров до точки
// “от же алгоритм только в обратную сторону
...
double SlopeNorm=0;
int PageStart=0;
// —читаем средний уклон на MinLen метров после точки
for (int i=0;i<FCount;i++)
    {
    int DL=L[i]-L[i-1];
